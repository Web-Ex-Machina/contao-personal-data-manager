...
modeUp(){
    ...
    then
        ...
        echo -e "> docker exec pdm_php_tu ../pdm/vendor/bin/codecept run unit";
    elif [[ "$type" = "test" ]]
    then
        echo -e "To run tests, run the following command :"
        echo -e "> docker exec pdm_php_test ../pdm/vendor/bin/codecept run --steps -c ../pdm/codeception.yml --no-redirect  --env firefox --env chrome";
    fi
}
